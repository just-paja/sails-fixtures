 ...
 */
var load = function(data,next){
	if ((!data) || (typeof data!="object")) next(new Error("Missing data parameter"));
	else prepare_fixture_job(data)(next);
}
 ...
 */
var clear = function(collections,next){
	if (!collections) next(new Error("Missing collections parameter"));
	else{
		collections = (typeof collections=="object") ? collections : [collections];
		async.parallel(collections.map(function(collection){
 ...
 */
var loadFromFiles = function(config, next)
{
	var jobs = [];
...
var clearCollection = function(collection){
	return function(next){
		if (typeof sails.models[collection] != "object") next(new Error("invalid collection "+collection));
		else sails.models[collection].destroy().exec(next);
	}
}
